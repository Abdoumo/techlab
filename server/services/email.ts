...
export const sendServiceInquiryEmail = async (
  serviceName: string,
  inquiryData: ServiceInquiry
) => {
  try {
    const transporter = createEmailTransporter();

    // Format the inquiry data for display
    ...
    const mailOptions = {
      ...
      to: process.env.GMAIL_EMAIL, // Send to the admin email
...
export const sendCustomerConfirmationEmail = async (email: string, serviceName: string) => {
  try {
    const transporter = createEmailTransporter();
